...
<body class="bg-slate-900">
    ...
    <script>
        ...
        <div class="flex flex-col h-full rounded-lg shadow-lg overflow-hidden">
            ...
            <div class="px-5 py-3 bg-gray-100 dark:bg-gray-700">
                <a href="#" onclick="confirmUnenrollment(${course.course_id}, this); return false;"
                class="unenroll-button inline-flex items-center w-full justify-center px-3 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                    Unenroll Now
        ...
        function fetchCourses() {
            return new Promise((resolve, reject) => {
                ...
                xhr.open('GET', '../backend/fetch_enrolled_courses.php', true);
                xhr.onload = function() {
                    if (this.status == 200) {
    ...
    <script>
        function confirmUnenrollment(courseId, element) {
            if (confirm('Are you sure you want to unenroll from this course?')) {
                // Proceed with unenrollment
                unenrollCourse(courseId, element);
            }
        }

        function unenrollCourse(courseId, element) {
            // AJAX request to backend script for unenrollment
            var xhr = new XMLHttpRequest();
            xhr.open('POST', '../backend/unenroll.php', true);
            xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
            xhr.onload = function() {
                if (this.status == 200) {
                    // On success, remove or update the course card or provide feedback
                    element.closest('.max-w-sm').remove(); // Remove the course card from the DOM
                } else {
                    // On error, provide feedback to the user
